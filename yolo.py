...
class Yolo:
    ...
    def __init__(self,
        ...
        if self.outputFolder is None:
            self.outputFolder = 'test/result/'
        if mode == 'testDB' and saveAnnotatedImage:
            if not os.path.exists("/".join(self.outputFolder.split("/")[:-1])):
...
def main():
    yolo = Yolo(mode='testVideo',inputFile='yolo_video.mp4',outputFile='video_output.mp4',inputFolder='test/demo/',outputFolder='test/result/',saveAnnotatedImage=True,iouThreshold=0.2,verbose=False)
